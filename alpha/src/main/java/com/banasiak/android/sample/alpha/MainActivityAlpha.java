...
public class MainActivityAlpha extends AppCompatActivity implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener {

    private static final String TAG = MainActivityAlpha.class.getSimpleName();
    ...
    void onAddButtonClick() {
        ...
        Auth.CredentialsApi.save(googleApiClient, credential).setResultCallback(new ResultCallback<Status>() {
            ...
            public void onResult(@NonNull Status status) {
                if (status.isSuccess()) {
                    Toast.makeText(MainActivityAlpha.this, "credentials saved", Toast.LENGTH_SHORT).show();
                } else {
                    if (status.hasResolution()) {
                        try {
                            // try to resolve the save request and prompt the user if the credential is new
                            status.startResolutionForResult(MainActivityAlpha.this, RC_SAVE_CREDENTIAL);
                        } catch (IntentSender.SendIntentException e) {
                            Toast.makeText(MainActivityAlpha.this, "save failed with resolution exception :(", Toast.LENGTH_SHORT).show();
                        }
                    } else {
                        Toast.makeText(MainActivityAlpha.this, "saved failed with no resolution :(", Toast.LENGTH_SHORT).show();
                    }
                }
            }
        });
